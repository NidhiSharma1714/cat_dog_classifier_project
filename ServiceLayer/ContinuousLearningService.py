...
the model has been updated we regenerate the TFLite artefact so future
inferences pick up the fresh weights.
"""
...
MODEL_DIR = PROJECT_ROOT / "repository" / "model_repository"
DATASET_DIR = (
    PROJECT_ROOT / "repository" / "dataset_repository" / "dataset" / "continuous_buffer"
)
...
class ContinuousLearningConfig:
    keras_model_path: Path = MODEL_DIR / "saved_model" / "cat_dog_classifier.h5"
    tflite_model_path: Path = MODEL_DIR / "cat_dog_classifier.tflite"
    buffer_dir: Path = DATASET_DIR
...
class ContinuousLearner:
    ...
    def _train_and_refresh(self) -> bool:
        ...
        steps = max(
            1,
            min(train_flow.samples // config.batch_size * config.augment_factor, config.augment_factor),
        )
        ...
        config.keras_model_path.parent.mkdir(parents=True, exist_ok=True)
